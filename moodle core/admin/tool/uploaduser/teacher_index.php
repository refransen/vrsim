...
$myCourseID  = optional_param('courselist',  0, PARAM_INT);    // The id of the course chosen from the selection box
$myCourse = array();    // The actual course information that belongs to the id

@set_time_limit(60*60); // 1 hour should be enough
raise_memory_limit(MEMORY_HUGE);

require_login();
teacher_uploadUsers_setup('tooluploaduser');
...
$returnurl = new moodle_url('/admin/tool/uploaduser/teacher_index.php');
...
if (empty($iid)) {
    ...
    if ($formdata = $mform1->get_data()) {
        ...
        // Rachel Fransen - assigned value to course selector box
        //-------------------------------------
        $mycourseID = $formdata->courselist;     
        $myCourse = $DB->get_record('course', array('id'=>$myCourseID));
...
$mform2 = new admin_uploaduser_form2(null, array('columns'=>$filecolumns, 'data'=>array('iid'=>$iid, 'previewrows'=>$previewrows, 'courselist'=>$myCourseID)));
...
if ($formdata = $mform2->is_cancelled()) {
    ...
    while ($line = $cir->next()) {
        ...
        if ($optype == UU_USER_ADDNEW or $optype == UU_USER_ADDINC) {
            ...
            // Rachel Fransen
            // Since the template function is broken in all Moodle 2.0, 
            // here is a function that defaults to my own template
            //----------------------------------
            if(empty($user->username))
            {
                $newFirstName = substr($user->firstname, 0, 6);
                $newLastName = substr($user->lastname, 0, 7);
                $user->username = $newFirstName.'_'.$newLastName;
        ...
        foreach ($filecolumns as $column) {
            ...
            // Added functionality to check the "course" select box on the upload form
            $myCourse = $DB->get_record('course', array('id'=>$myCourseID));
...
// Print out an extra header that describes the course you're uploading for
//-------------------------------------
if(!empty($myCourse))
{
   echo '<div class="courseUpdateText" >You are uploading users for course: <span>"'.$myCourse->shortname.'"</span></div>';
}
else
{
   echo '<div class="courseUpdateText" >These users will not be enrolled in any course</div>';
}
...
while ($linenum <= $previewrows and $fields = $cir->next()) {
    ...
    if (isset($rowcols['username'])) 
    {
        ...
        if(empty($stdusername))
        {
            $rowcols['status'][] = "Username will be generated";
        }
    } 
    else 
    {
