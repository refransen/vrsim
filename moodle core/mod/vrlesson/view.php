...
/// These can be added to perform an action on a record
    ...
    /////////////////////////////////////
    // Custom function to sort records according
    // to time
    ////////////////////////////////////
...
/// Check further parameters that set browsing preferences
    ...
    if (!isset($SESSION->dataprefs[$data->id])) {
        ...
        $SESSION->dataprefs[$data->id]['entries'] = $data->defaultnumentries;
    }
    ...
    if (!empty($advanced)) {
        ...
        if (!empty($fields)) {
            foreach($fields as $field) {
                $searchfield = vrlesson_get_field_from_id($field->id, $data);
        ...
        if (!$paging) {
            ...
            $fn = isset($search_array[VRLESSON_FIRSTNAME]) ? $search_array[VRLESSON_FIRSTNAME]->data : '';
            $ln = isset($search_array[VRLESSON_LASTNAME]) ? $search_array[VRLESSON_LASTNAME]->data : '';
        }
        if (!empty($fn)) {
            $search_array[VRLESSON_FIRSTNAME] = new stdClass();
            $search_array[VRLESSON_FIRSTNAME]->sql    = '';
            $search_array[VRLESSON_FIRSTNAME]->params = array();
            $search_array[VRLESSON_FIRSTNAME]->field  = 'u.firstname';
            $search_array[VRLESSON_FIRSTNAME]->data   = $fn;
            $vals[] = $fn;
        } else {
            unset($search_array[VRLESSON_FIRSTNAME]);
        }
        if (!empty($ln)) {
            $search_array[VRLESSON_LASTNAME] = new stdClass();
            $search_array[VRLESSON_LASTNAME]->sql     = '';
            $search_array[VRLESSON_LASTNAME]->params = array();
            $search_array[VRLESSON_LASTNAME]->field   = 'u.lastname';
            $search_array[VRLESSON_LASTNAME]->data    = $ln;
            $vals[] = $ln;
        } else {
            unset($search_array[VRLESSON_LASTNAME]);
        }
    ...
    // Rachel Fransen - Added this option
    $numEntries = optional_param('entries', $SESSION->dataprefs[$data->id]['entries'], PARAM_INT); 
    $SESSION->dataprefs[$data->id]['entries'] = $numEntries;     // Make it sticky
        
    $oldperpage = get_user_preferences('vrlesson_perpage_'.$data->id, 10);
    ...
    if ($perpage != $oldperpage) {
        set_user_preference('vrlesson_perpage_'.$data->id, $numEntries);
    }

    add_to_log($course->id, 'vrlesson', 'view', "view.php?id=$cm->id", $data->id, $cm->id);
...
// Initialize $PAGE, compute blocks
    $PAGE->set_url('/mod/vrlesson/view.php', $urlparams);
...
/// RSS and CSS and JS meta
    ...
    if (!empty($CFG->enablerssfeeds) && !empty($CFG->vrlesson_enablerssfeeds) && $data->rssarticles > 0) {
        ...
        rss_add_http_header($context, 'mod_vrlesson', $data, $rsstitle);
    }
    if ($data->csstemplate) {
        $PAGE->requires->css('/mod/vrlesson/css.php?d='.$data->id);
    }
    if ($data->jstemplate) {
        $PAGE->requires->js('/mod/vrlesson/js.php?d='.$data->id, true);
    }
...
/// Print the page header
    ...
    if ($PAGE->user_allowed_editing()) {
        ...
        $url = new moodle_url($CFG->wwwroot.'/mod/vrlesson/view.php', array('id' => $cm->id, 'edit' => $urlediting));
...
/// Check to see if groups are being used here
    $returnurl = $CFG->wwwroot . '/mod/vrlesson/view.php?d='.$data->id.'&amp;search='.s($search).'&amp;sort='.s($sort).'&amp;order='.s($order).'&amp;';
    ...
    if ($currentgroup == 0 && $groupmode == 1 && !has_capability('mod/vrlesson:manageentries', $context)) {
    ...
    {
        if($record and $data->approval and !$record->approved and $record->userid != $USER->id) {
            if (!$currentgroup or $record->groupid == $currentgroup or $record->groupid == 0) 
            {
                print_error('notapproved', 'vrlesson');
            }
        }
    }
    ...
    //this links has been Settings (vrlesson activity administration) block
    /*if (!empty($CFG->enablerssfeeds) && !empty($CFG->vrlesson_enablerssfeeds) && $data->rssarticles > 0) {
        echo '<div style="float:right;">';
        rss_print_link($context->id, $USER->id, 'mod_vrlesson', $data->id, get_string('rsstype'));
        echo '</div>';
    ...
    if ($data->intro and empty($page) and empty($record) and $mode != 'single') {
        ...
        echo $OUTPUT->box(format_module_intro('vrlesson', $data, $cm->id), 'generalbox', 'intro');
    }

/// Delete any requested records

    if ($delete && confirm_sesskey() && (has_capability('mod/vrlesson:manageentries', $context) or vrlesson_isowner($delete))) {
        if ($confirm = optional_param('confirm',0,PARAM_INT)) {
            if ($deleterecord = $DB->get_record('vrlesson_records', array('id'=>$delete))) {   // Need to check this is valid
                if ($deleterecord->dataid == $data->id) {                       // Must be from this database
                    if ($contents = $DB->get_records('vrlesson_content', array('recordid'=>$deleterecord->id))) {
                        foreach ($contents as $content) {  // Delete files or whatever else this field allows
                            if ($field = vrlesson_get_field_from_id($content->fieldid, $data)) { // Might not be there
                                $field->delete_content($content->recordid);
                            }
                        }
                    }
                    $DB->delete_records('vrlesson_content', array('recordid'=>$deleterecord->id));
                    $DB->delete_records('vrlesson_records', array('id'=>$deleterecord->id));

                    add_to_log($course->id, 'vrlesson', 'record delete', "view.php?id=$cm->id", $data->id, $cm->id);

                    echo $OUTPUT->notification(get_string('recorddeleted','vrlesson'), 'notifysuccess');
                }
            }
            ...
            if ($deleterecord = $DB->get_record('vrlesson_records', array('id'=>$delete))) {   // Need to check this is valid
                if ($deleterecord->dataid == $data->id) {                       // Must be from this database
                    $deletebutton = new single_button(new moodle_url('/mod/vrlesson/view.php?d='.$data->id.'&delete='.$delete.'&confirm=1'), get_string('delete'), 'post');
                    echo $OUTPUT->confirm(get_string('confirmdeleterecord','vrlesson'),
                            $deletebutton, 'view.php?d='.$data->id);

                    $records[] = $deleterecord;
                    echo vrlesson_print_template('singletemplate', $records, $data, '', 0, true);
...
if (!has_capability('mod/vrlesson:manageentries', $context)) {
    ...
    if (!empty($data->timeavailablefrom) && $data->timeavailablefrom > $timenow) {
        echo $OUTPUT->notification(get_string('notopenyet', 'vrlesson', userdate($data->timeavailablefrom)));
        $showactivity = false;
    } else if (!empty($data->timeavailableto) && $timenow > $data->timeavailableto) {
        echo $OUTPUT->notification(get_string('expired', 'vrlesson', userdate($data->timeavailableto)));
        $showactivity = false;
    }
}
...
if ($showactivity) {
    ...
    /// Approve any requested records
        ...
        $approvecap = has_capability('mod/vrlesson:approve', $context);

        if ($approve && confirm_sesskey() && $approvecap) {
            if ($approverecord = $DB->get_record('vrlesson_records', array('id'=>$approve))) {   // Need to check this is valid
                if ($approverecord->dataid == $data->id) {                       // Must be from this database
                    $newrecord = new stdClass();
                    $newrecord->id = $approverecord->id;
                    $newrecord->approved = 1;
                    $DB->update_record('vrlesson_records', $newrecord);
                    echo $OUTPUT->notification(get_string('recordapproved','vrlesson'), 'notifysuccess');
                }
            }
        }
    ...
    /// Check the number of entries required against the number of entries already made (doesn't apply to teachers)
        if ($data->requiredentries > 0 && $numentries < $data->requiredentries && !has_capability('mod/vrlesson:rate', $context)) {
            $data->entriesleft = $data->requiredentries - $numentries;
            $strentrieslefttoadd = get_string('entrieslefttoadd', 'vrlesson', $data);
    ...
    /// Check the number of entries required before to view other participant's entries against the number of entries already made (doesn't apply to teachers)
        ...
        if ($data->requiredentriestoview > 0 && $numentries < $data->requiredentriestoview && !has_capability('mod/vrlesson:manageentries', $context)) {
            $data->entrieslefttoview = $data->requiredentriestoview - $numentries;
            $strentrieslefttoaddtoview = get_string('entrieslefttoaddtoview', 'vrlesson', $data);
    ...
    /// Find the field we are sorting on
        if ($sort <= 0 or !$sortfield = vrlesson_get_field_from_id($sort, $data)) {
            ...
            $tables = '{vrlesson_content} c,{vrlesson_records} r, {user} u ';
            ...
            $params['dataid'] = $data->id;
            $params['sort'] = $sort;
            $sortorder = ' ORDER BY sortorder '.$order.' , r.id DESC';
            $searchselect = '';
    ...
    /// To actually fetch the records
        ...
        $recordids = vrlesson_get_all_recordids($data->id, $initialselect, $initialparams);
        $newrecordids = vrlesson_get_advance_search_ids($recordids, $search_array, $data->id);
        $totalcount = count($newrecordids);
        $selectdata = $where . $groupselect . $approveselect;

        if (!empty($advanced)) {
            $advancedsearchsql = vrlesson_get_advanced_search_sql($sort, $data, $newrecordids, $selectdata, $sortorder);
    ...
    /// Get the actual records
        ...
        if (!$records = $DB->get_records_sql($sqlselect, $allparams, $page * $nowperpage, $nowperpage)) {
            ...
            if ($record) {         // Something was requested so try to show that at least (bug 5132)
                if (has_capability('mod/vrlesson:manageentries', $context) || empty($data->approval) ||
        ...
        if (empty($records)) {
            if ($maxcount){
                ...
                echo $OUTPUT->notification(get_string('foundnorecords','vrlesson', $a));
            } else {
                echo $OUTPUT->notification(get_string('norecords','vrlesson'));
            }
            ...
            if ($maxcount != $totalcount) {
                ...
                $a->max = $maxcount;
                $a->reseturl = "view.php?id=$cm->id&amp;mode=$mode&amp;search=&amp;advanced=0";
                echo $OUTPUT->notification(get_string('foundrecords', 'vrlesson', $a), 'notifysuccess');
            }
            ...
            if ($mode == 'single') { 
                ...
                // Rachel Fransen - March 31, 2013
                // Only show top paging bar if admin user
                if(is_siteAdmin($USER))  {
                   echo $OUTPUT->paging_bar($totalcount, $page, $nowperpage, $baseurl);
                }

                if (empty($data->singletemplate)){
                    echo $OUTPUT->notification(get_string('nosingletemplate','vrlesson'));
                    vrlesson_generate_default_template($data, 'singletemplate', 0, false, false);
                }

                //vrlesson_print_template() only adds ratings for singletemplate which is why we're attaching them here
                //attach ratings to data records
                require_once($CFG->dirroot.'/rating/lib.php');
                if ($data->assessed != RATING_AGGREGATE_NONE) 
                {
                    // Create an auto-rating system based on uploaded score
                    foreach($records as $myRecord)
                    {
                       $scoreField = $DB->get_record('vrlesson_fields', array('dataid'=>$data->id, 'name'=>'Score'));
                       $currDBScore = $DB->get_record('vrlesson_content', 
                                      array('recordid'=>$myRecord->id, 'fieldid'=>$scoreField->id));

                       // get rating scale max
                       // Our scores are based on 100, while scale is set to 70, 80, etc
                       // We need to reset the score based on our own scale
                       $newScore = (int)$currDBScore->content;
                       if((int)$currDBScore->content > $data->scale)
                       {  $newScore = $data->scale;  }

                       // only update the score if it's different and higher
                       if($currentRating = $DB->get_record('rating', array('itemid'=>$myRecord->id)))  {
                       	   if($newScore !== $currentRating->rating) {                       
	                   	$currentRating->rating = $newScore;
	                    	$DB->update_record('rating', $currentRating);
                       	   }
                       }
                    }

                    // Display the rating options
                    ...
                    $ratingoptions->returnurl = $CFG->wwwroot.'/mod/vrlesson/'.$baseurl;
                ...
                vrlesson_print_template('singletemplate', $records, $data, $search, $page);
            ...
            //-----------------------------------
                ...
                if (empty($data->listtemplate)){
                    echo $OUTPUT->notification(get_string('nolisttemplate','vrlesson'));
                    vrlesson_generate_default_template($data, 'listtemplate', 0, false, false);
                }

                // Rachel Fransen - March 31, 2013
                // Parse the records so we don't have duplicate names
                // ONly show 3
                //--------------------------------------------------------
                $newCSV = new vrlesson_csv();
                $nameField = $DB->get_record('vrlesson_fields', array('dataid'=>$data->id, 'name'=>'Project Name'));
                $passesField = $DB->get_record('vrlesson_fields', array('dataid'=>$data->id, 'name'=>'Passes'));
                ...
                $nameResults = array_unique($projNameArray);
                
                // Get the highest score of each project
                //--------------------------------------------------------
                $isteacher = has_capability('mod/vrlesson:rate', $context);
                $newRecordArray = array();
                foreach($nameResults as $singleName)
                {
                    $newCSV->get_highest_score($singleName, $USER->id , $data->id, false);
                    $entryID = $newCSV->get_highest_entryid();

                    if($defaultRecord = $DB->get_record('vrlesson_records', array('dataid'=>$data->id, 'id'=>$entryID)))
                    {  $newRecordArray[] = $defaultRecord; }
                    else
                    {
                        $newRecordArray[] = $newCSV->get_default_project($singleName, $data->id);
                    }
                }

                // Populate a new list of entries for a search
                //-------------------------------------------------
                $projNameArray = array();
                if(!empty($search)) 
                {  
                    arsort($records);
                    foreach($records as $singleRec)
                    {
                       $tempName = $DB->get_record('vrlesson_content', array('recordid'=>$singleRec->id, 'fieldid'=>$nameField->id));
                       $projNameArray[$singleRec->id] = $tempName->content; 
                    }
                    $nameResults = array_unique($projNameArray);

                    $myTempRecs = array();
                    foreach($nameResults as $key => $val)
                    {
                         $myTempRecs[] = $DB->get_record('vrlesson_records', array('id'=>$key));
                    }

                    $newRecordArray = $myTempRecs;
                    $nowperpage = 10;
                }
                ...
                vrlesson_print_template('listtemplate', $newRecordArray, $data, $search, $page);
                echo $data->listtemplatefooter;

                // Rachel Fransen - April 1, 2013
                // Only show this to the admin
                if(is_siteAdmin($USER)) {
                    echo $OUTPUT->paging_bar($totalcount, $page, $nowperpage, $baseurl);
                }
            }
    ...
    if ($mode == '' && !empty($CFG->enableportfolios) && !empty($records)) {
        ...
        $button->set_callback_options('vrlesson_portfolio_caller', array('id' => $cm->id), 'mod_vrlesson');
        if (vrlesson_portfolio_caller::has_files($data)) {
    ...
    if (($maxcount || $mode == 'asearch') && $mode != 'single') 
    {
         ...
         // Added this code to hide paging bar to normal users
         // Also displaying my custom field numbers here
         if(is_siteadmin($USER)){
             vrlesson_print_preference_form($data, $numEntries, $search, $sort, $order, $search_array, $advanced, $mode);
         }
    }
}
