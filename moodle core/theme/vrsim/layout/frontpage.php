...
// If the teacher tries to come to this page, redirect them
//---------------------------------
$urltogo = $CFG->wwwroot;
if ($courses = enrol_get_my_courses(NULL, 'visible DESC, fullname ASC')) {
    foreach($courses as $course)  {
        $context = context_course::instance(3);
        ...
        foreach($roleList as $singleRole)
        {
             if($singleRole->roleid < 5 && !is_siteadmin() && !isguestuser()) { 
                  ...
                  redirect(new moodle_url($urltogo, array('redirect' => 0)));
                  break;
              }
        }
    }
...
//---------------------------------------
 ...
 echo '<link rel="stylesheet" type="text/css" href="'.$CFG->wwwroot.'/theme/vrsim/style/frontpage.css">';
 echo '<script type="text/javascript" src="'.$CFG->wwwroot.'/theme/vrsim/javascript/avatarpage.js"></script>';
