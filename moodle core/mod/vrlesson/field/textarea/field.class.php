...
class vrlesson_field_textarea extends vrlesson_field_base {
    ...
    function display_add_field($recordid=0) {
        ...
        if ($recordid && $content = $DB->get_record('vrlesson_content', array('fieldid'=>$this->field->id, 'recordid'=>$recordid))){
            $format = $content->content1;
            $text = clean_text($content->content, $format);
            $text = file_prepare_draft_area($draftitemid, $this->context->id, 'mod_vrlesson', 'content', $content->id, $options, $text);
        } else {
    ...
    function update_content($recordid, $value, $name='') {
        ...
        if ($oldcontent = $DB->get_record('vrlesson_content', array('fieldid'=>$this->field->id, 'recordid'=>$recordid))) {
            ...
            $content->id = $DB->insert_record('vrlesson_content', $content);
            if (!$content->id) {
                return false;
            }
        }
        if (!empty($content->content)) {
            ...
            $options = $this->get_options();
            $content->content = file_save_draft_area_files($draftitemid, $this->context->id, 'mod_vrlesson', 'content', $content->id, $options, $content->content);
        }
        $rv = $DB->update_record('vrlesson_content', $content);
        return $rv;
    }
    ...
    function display_browse_field($recordid, $template) {
        ...
        if ($content = $DB->get_record('vrlesson_content', array('fieldid' => $this->field->id, 'recordid' => $recordid))) {
            if (isset($content->content)) {
                ...
                $options->para = false;
                $str = file_rewrite_pluginfile_urls($content->content, 'pluginfile.php', $this->context->id, 'mod_vrlesson', 'content', $content->id, $this->get_options());
