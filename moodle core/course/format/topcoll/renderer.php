...
class format_topcoll_renderer extends format_section_renderer_base {
    ...
    protected function section_header($section, $course, $onsectionpage, $sectionreturn = null) {
        ...
        if ($section->section != 0) 
        {
            ...
            {
                // Check all activities and see if they are complete. If so, make that section green
                $myactivities = get_array_of_activities($course->id);
                $numSectActivities = 0;
                $completeCounter = 0;
                foreach($myactivities as $activity)
		{
		     if($activity->sectionid == $section->id && $activity->mod == 'vrlesson')  
		     {	
                         $numSectActivities++;
                         if($DB->get_record('course_modules_completion', array('userid'=>$USER->id, 'coursemoduleid'=>$activity->cm)))
                         {  $completeCounter++;   }
                     }
                }
                if($numSectActivities > 0 && $completeCounter >= $numSectActivities)
                {
                    $sectionstyle = ' complete';
                    $rightcurrent = ' left';
                }
                else
                {
