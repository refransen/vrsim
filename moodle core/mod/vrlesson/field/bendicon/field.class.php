...
class vrlesson_field_bendicon extends vrlesson_field_base {
    var $type = 'bendicon';
    var $previewwidth  = 50;
    ...
    function display_add_field($recordid=0) {
        ...
        if ($recordid) {
            if ($content = $DB->get_record('vrlesson_content', array('fieldid'=>$this->field->id, 'recordid'=>$recordid))) {
                file_prepare_draft_area($itemid, $this->context->id, 'mod_vrlesson', 'content', $content->id);
                if (!empty($content->content)) {
                    if ($file = $fs->get_file($this->context->id, 'mod_vrlesson', 'content', $content->id, '/', $content->content)) {
        ...
        if ($file) {
            $src = file_encode_url($CFG->wwwroot.'/pluginfile.php/', $this->context->id.'/mod_vrlesson/content/'.$content->id.'/'.$file->get_filename());
        ...
        $str .= '<label for="field_'.$this->field->id.'_alttext">'.get_string('alttext','vrlesson') .'</label>&nbsp;<input type="text" name="field_'
        ...
        $module = array('name'=>'vrlesson_imagepicker', 'fullpath'=>'/mod/vrlesson/data.js', 'requires'=>array('core_filepicker'));
        $PAGE->requires->js_init_call('M.vrlesson_imagepicker.init', array($fp->options), true, $module);
        return $str;
    }

    // TODO delete this function and instead subclass vrlesson_field_file - see MDL-16493
    ...
    function display_search_field($value = '') {
        return '<label class="accesshide" for="f_'.$this->field->id.'">' . get_string('fieldname', 'vrlesson') . '</label>' .
    ...
    function display_browse_field($recordid, $template) {
        ...
        //------------------
        // Parse bend information
        //------------------
        $bendFieldID = $DB->get_record('vrlesson_fields', array('dataid'=>$this->field->dataid, 'name'=>'Bend Tests'));
        $bendFieldContent = $DB->get_record('vrlesson_content', array('fieldid'=>$bendFieldID->id, 'recordid'=>$recordid));
    	$finalBendContent = explode("\n", $bendFieldContent->content);
    	
    	$passCounter = 0;
    	foreach($finalBendContent as $myContent)
    	{
    	     $pos = strpos($myContent, "Reason");
    	     if($pos !== false)
    	     {
    	         $reasonStr = substr($myContent, $pos);
    	         if(!empty($reasonStr) && $reasonStr !== 'None')
    	         {  $passCounter++;  }
    	     }
    	}
    	
    	//------------------
        // Determine if the user passed all bends
        //------------------
    	$isBendPassed = false;
    	$totalBendCount = 4;
    	$numOfBends = count($finalBendContent);
    	if($numOfBends >= $totalBendCount && $passCounter >= $totalBendCount)
    	{
    	    $isBendPassed = true;
    	}
	...
	//------------------
        if ($template == 'listtemplate') {
            $src = file_encode_url($CFG->wwwroot.'/pluginfile.php', '/'.$this->context->id.'/mod_vrlesson/content/'.$content->id.'/'.'thumb_'.$content->content);
            // no need to add width/height, because the thumb is resized properly
            $str = '<a href="view.php?d='.$this->field->dataid.'&amp;rid='.$recordid.'"><img src="'.$src.'" alt="'.s($alt).'" title="'.s($title).'" class="list_bendicon"/></a>';

        } else 
        {
            $imgSrc = $CFG->wwwroot.'/theme/vrsim/pix_plugins/mod/vrlesson/field/bendicon/Bend_NotPassed_Icon.png';
            $title = 'Bend tests not passed';
            $alt = 'Bend tests not passed';
            if($isBendPassed)
            {
                 $imgSrc = $CFG->wwwroot.'/theme/vrsim/pix_plugins/mod/vrlesson/field/bendicon/Bend_Passed_Icon.png';
                 $title = 'Bend Tests Passed';
                 $alt = 'Bend Tests Passed';
            }
            $src = file_encode_url($CFG->wwwroot.'/pluginfile.php', '/'.$this->context->id.'/mod_vrlesson/content/'.$content->id.'/'.$content->content);
            $width  = $this->field->param1 ? ' width="'.s($this->field->param1).'" ':' ';
            $height = $this->field->param2 ? ' height="'.s($this->field->param2).'" ':' ';
            $str = '<a href="'.$src.'"><img '.$width.$height.' src="'.$imgSrc.'" alt="'.s($alt).'" title="'.s($title).'" class="list_bendicon"/></a>';
        }
    ...
    function update_field() {
        ...
        $oldfield = $DB->get_record('vrlesson_fields', array('id'=>$this->field->id));
        ...
        if ($oldfield && ($oldfield->param4 != $this->field->param4 || $oldfield->param5 != $this->field->param5)) {
            ...
            if ($contents = $DB->get_records('vrlesson_content', array('fieldid'=>$this->field->id))) {
                $fs = get_file_storage();
                if (count($contents) > 20) {
                    echo $OUTPUT->notification(get_string('resizingimages', 'vrlesson'), 'notifysuccess');
                    echo "\n\n";
                ...
                foreach ($contents as $content) {
                    ...
                    if ($thumbfile = $fs->get_file($this->context->id, 'mod_vrlesson', 'content', $content->id, '/', 'thumb_'.$content->content)) {
                        $thumbfile->delete();
                    }
                    @set_time_limit(300);
                    // Might be slow!
    ...
    function update_content($recordid, $value, $name='') {
        ...
        // Quickly make one now!
            ...
            $id = $DB->insert_record('vrlesson_content', $content);
            $content = $DB->get_record('vrlesson_content', array('id'=>$id));
        }
            ...
            case 'file':
                ...
                if (count($files)<2) {
                    ...
                    foreach ($files as $draftfile) {
                        $file_record = array('contextid'=>$this->context->id, 'component'=>'mod_vrlesson', 'filearea'=>'content', 'itemid'=>$content->id, 'filepath'=>'/');
