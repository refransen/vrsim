...
/// Print overview
echo '<h3 style="text-align:center;" >'.format_string($course->shortname, true, array('context' => $context)) .' '.$stroverview.'</h3>';
echo $strfiltergroups.'<br />';

// Print out the groups select blox
$options = array();
...
// Print out the groupings select box
$options = array();
$options[0] = get_string('all');
foreach ($groupings as $grouping) {
...
foreach ($members as $gpgid=>$groupdata) {
    ...
    if ($gpgid < 0) {
        // Rachel Fransen - Commented this out because the text is confusing
        //echo $OUTPUT->heading($strnotingrouping, 3);
    } else {
        // Rachel Fransen - Commented this text out because it's confusing
