...
function showHideAdvSearch(checked) {
    ...
    for(i=0;i<divs.length;i++) {
        if(divs[i].id.match('vrlesson_adv_form')) {
            if(checked) {
...
M.vrlesson_filepicker = {};


M.vrlesson_filepicker.callback = function(params) {
 ...
 */
M.vrlesson_filepicker.init = function(Y, options) {
    options.formcallback = M.vrlesson_filepicker.callback;
...
M.vrlesson_urlpicker = {};

M.vrlesson_urlpicker.init = function(Y, options) {
    options.formcallback = M.vrlesson_urlpicker.callback;
    if (!M.core_filepicker.instances[options.client_id]) {
        M.core_filepicker.init(Y, options);
    }
    Y.on('click', function(e, client_id) {
...
M.vrlesson_urlpicker.callback = function (params) {
    document.getElementById('field_url_'+params.client_id).value = params.url;
};

M.vrlesson_imagepicker = {};

M.vrlesson_imagepicker.callback = function(params) {
 ...
 * This fucntion is called for each file picker on page.
 */
M.vrlesson_imagepicker.init = function(Y, options) {
    options.formcallback = M.vrlesson_imagepicker.callback;
    if (!M.core_filepicker.instances[options.client_id]) {
        M.core_filepicker.init(Y, options);
    }
    Y.on('click', function(e, client_id) {
