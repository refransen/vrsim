...
/**
 * The class to handle entry exports of a vrlesson module
 */
class vrlesson_portfolio_caller extends portfolio_module_caller_base {
    ...
    public function load_data() {
        global $DB, $USER;
        if (!$this->cm = get_coursemodule_from_id('vrlesson', $this->id)) {
            throw new portfolio_caller_exception('invalidid', 'vrlesson');
        }
        if (!$this->data = $DB->get_record('vrlesson', array('id' => $this->cm->instance))) {
            throw new portfolio_caller_exception('invalidid', 'vrlesson');
        }
        $fieldrecords = $DB->get_records('vrlesson_fields', array('dataid' => $this->cm->instance), 'id');
        // populate objets for this databases fields
        $this->fields = array();
        foreach ($fieldrecords as $fieldrecord) {
            $tmp = vrlesson_get_field($fieldrecord, $this->data);
            $this->fields[] = $tmp;
        ...
        $this->records = array();
        if ($this->recordid) {
            $tmp = $DB->get_record('vrlesson_records', array('id' => $this->recordid));
            $tmp->content = $DB->get_records('vrlesson_content', array('recordid' => $this->recordid));
            $this->records[] = $tmp;
        } else {
            $where = array('dataid' => $this->data->id);
            if (!has_capability('mod/vrlesson:exportallentries', context_module::instance($this->cm->id))) {
                $where['userid'] = $USER->id; // get them all in case, we'll unset ones that aren't ours later if necessary
            }
            $tmp = $DB->get_records('vrlesson_records', $where);
            foreach ($tmp as $t) {
                $t->content = $DB->get_records('vrlesson_content', array('recordid' => $t->id));
                $this->records[] = $t;
            }
            $this->minecount = $DB->count_records('vrlesson_records', array('dataid' => $this->data->id, 'userid' => $USER->id));
        }
        ...
        if ($leapwriter) {
            if (count($this->records) > 1) { // make a selection element to tie them all together
                $selection = new portfolio_format_leap2a_entry('datadb' . $this->data->id,
                    get_string('entries', 'vrlesson') . ': ' . $this->data->name, 'selection');
    ...
    public function check_permissions() {
        if ($this->recordid) {
            if (vrlesson_isowner($this->recordid)) {
                return has_capability('mod/vrlesson:exportownentry', context_module::instance($this->cm->id));
            }
            return has_capability('mod/vrlesson:exportentry', context_module::instance($this->cm->id));
        }
        if ($this->has_export_config() && !$this->get_export_config('mineonly')) {
            return has_capability('mod/vrlesson:exportallentries', context_module::instance($this->cm->id));
        }
        return has_capability('mod/vrlesson:exportownentry', context_module::instance($this->cm->id));
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'vrlesson');
    }
    ...
    public function __wakeup() {
        ...
        foreach ($this->fieldtypes as $key => $field) {
            require_once($CFG->dirroot . '/mod/vrlesson/field/' . $field .'/field.class.php');
    ...
    public static function has_files($data) {
        ...
        // populate objets for this databases fields
        foreach ($fieldrecords as $fieldrecord) {
            $field = vrlesson_get_field($fieldrecord, $data);
            if (is_callable(array($field, 'get_file'))) {
                return true;
            }
        }
    ...
    public function has_export_config() {
        ...
        return (empty($this->recordid) // multi-entry export
            ...
            && has_capability('mod/vrlesson:exportallentries', context_module::instance($this->cm->id))); // they actually have a choice in the matter
    }
    ...
    public function export_config_form(&$mform, $instance) {
        ...
        $mform->addElement('selectyesno', 'mineonly', get_string('exportownentries', 'vrlesson', (object)array('mine' => $this->minecount, 'all' => count($this->records))));
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class vrlesson_file_info_container extends file_info {
    /** @var file_browser */
    protected $browser;
    /** @var stdClass */
    protected $course;
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        foreach ($rs as $record) {
            if ($child = $this->browser->get_file_info($this->context, 'mod_vrlesson', $this->filearea, $record->itemid)) {
                $children[] = $child;
            }
