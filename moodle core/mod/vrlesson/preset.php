...
require_once($CFG->dirroot.'/mod/vrlesson/preset_form.php');
require_once($CFG->libdir.'/xmlize.php');

$id     = optional_param('id', 0, PARAM_INT);           // course module id
if ($id) {
    $cm = get_coursemodule_from_id('vrlesson', $id, null, null, MUST_EXIST);
    ...
    $cm = get_coursemodule_from_instance('vrlesson', $data->id, $course->id, null, MUST_EXIST);
}

$context = context_module::instance($cm->id, MUST_EXIST);
require_login($course, false, $cm);
require_capability('mod/vrlesson:managetemplates', $context);
$PAGE->set_url(new moodle_url('/mod/vrlesson/preset.php', array('d'=>$data->id)));
...
$presets = vrlesson_get_available_presets($context);
$strdelete = get_string('deleted', 'vrlesson');
foreach ($presets as &$preset) {
    if (!empty($preset->userid)) {
        ...
        if (vrlesson_user_can_delete_preset($context, $preset) && $preset->name != 'Image gallery') {
            $delurl = new moodle_url('/mod/vrlesson/preset.php', array('d'=> $data->id, 'action'=>'confirmdelete', 'fullname'=>$preset->userid.'/'.$preset->shortname, 'sesskey'=>sesskey()));
    ...
    if ($preset->userid > 0 && vrlesson_user_can_delete_preset($context, $preset)) {
        $delurl = new moodle_url('/mod/vrlesson/preset.php', array('d'=> $data->id, 'action'=>'confirmdelete', 'fullname'=>$preset->userid.'/'.$preset->shortname, 'sesskey'=>sesskey()));
        $delicon = html_writer::empty_tag('img', array('src'=>$OUTPUT->pix_url('t/delete'), 'class'=>'iconsmall', 'alt'=>$strdelete.' '.$preset->description));
        $preset->description .= html_writer::link($delurl, $delicon);
    }
...
$form_importexisting = new vrlesson_existing_preset_form(null, array('presets'=>$presets));
$form_importexisting->set_data(array('d' => $data->id));

$form_importzip = new vrlesson_import_preset_zip_form();
$form_importzip->set_data(array('d' => $data->id));

$form_export = new vrlesson_export_form();
$form_export->set_data(array('d' => $data->id));

$form_save = new vrlesson_save_preset_form();
...
if (optional_param('sesskey', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    $renderer = $PAGE->get_renderer('mod_vrlesson');

    if ($formdata = $form_importexisting->get_data()) {
        $importer = new vrlesson_preset_existing_importer($course, $cm, $data, $formdata->fullname);
        ...
        $importer = new vrlesson_preset_upload_importer($course, $cm, $data, $file->path);
        echo $renderer->import_setting_mappings($data, $importer);
        echo $OUTPUT->footer();
        exit(0);
        ...
        $exportfile = vrlesson_presets_export($course, $cm, $data);
        ...
        if (!empty($formdata->overwrite)) {
            ...
            if (isset($selectedpreset->name)) {
                if (vrlesson_user_can_delete_preset($context, $selectedpreset)) {
                    vrlesson_delete_site_preset($formdata->name);
                } else {
                    print_error('cannotoverwritepreset', 'vrlesson');
                }
            }
        }

        // If the preset exists now then we need to throw an error.
        $sitepresets = vrlesson_get_available_site_presets($context);
        ...
        // Save the preset now
        vrlesson_presets_save($course, $cm, $data, $formdata->name);

        echo $OUTPUT->notification(get_string('savesuccess', 'vrlesson'), 'notifysuccess');
        ...
        if ($userid && ($userid != $USER->id) && !has_capability('mod/vrlesson:viewalluserpresets', $context)) {
            print_error('cannotaccesspresentsother', 'vrlesson');
        }

        if ($action == 'confirmdelete') {
            $path = vrlesson_preset_path($course, $userid, $shortname);
            $strwarning = get_string('deletewarning', 'vrlesson').'<br />'.$shortname;
            ...
            if (!isset($selectedpreset->shortname) || !vrlesson_user_can_delete_preset($context, $selectedpreset)) {
               print_error('invalidrequest');
            }

            vrlesson_delete_site_preset($shortname);

            $strdeleted = get_string('deleted', 'vrlesson');
            ...
            if (!$fullname) {
                ...
                $importer = new vrlesson_preset_upload_importer($course, $cm, $data, $presetdir);
            } else {
                $importer = new vrlesson_preset_existing_importer($course, $cm, $data, $fullname);
            }
            $importer->import($overwritesettings);
            $strimportsuccess = get_string('importsuccess', 'vrlesson');
            $straddentries = get_string('addentries', 'vrlesson');
            $strtodatabase = get_string('todatabase', 'vrlesson');
            if (!$DB->get_records('vrlesson_records', array('dataid'=>$data->id))) {
...
// Export forms
echo $OUTPUT->heading(get_string('export', 'vrlesson'));
$form_export->display();
$form_save->display();
