...
$PAGE->set_url('/mod/vrlesson/index.php', array('id'=>$id));
...
add_to_log($course->id, "vrlesson", "view all", "index.php?id=$course->id", "");

$strsectionname  = get_string('sectionname', 'format_'.$course->format);
$strname = get_string('name');
$strdata = get_string('modulename','vrlesson');
$strdataplural  = get_string('modulenameplural','vrlesson');

$PAGE->navbar->add($strdata, new moodle_url('/mod/vrlesson/index.php', array('id'=>$course->id)));
$PAGE->set_title($strdata);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (! $datas = get_all_instances_in_course("vrlesson", $course)) {
...
$strentries = get_string('entries', 'vrlesson');
$strnumnotapproved = get_string('numnotapproved', 'vrlesson');
...
$rss = (!empty($CFG->enablerssfeeds) && !empty($CFG->vrlesson_enablerssfeeds));
...
foreach ($datas as $data) {
    ...
    $numrecords = $DB->count_records_sql('SELECT COUNT(r.id) FROM {vrlesson_records} r WHERE r.dataid =?', array($data->id));

    if ($data->approval == 1) {
        $numunapprovedrecords = $DB->count_records_sql('SELECT COUNT(r.id) FROM {vrlesson_records} r WHERE r.dataid =? AND r.approved <> 1', array($data->id));
    } else {
    ...
    if ($rss && $data->rssarticles > 0) {
        $rsslink = rss_get_link($context->id, $USER->id, 'mod_vrlesson', $data->id, 'RSS');
    }
