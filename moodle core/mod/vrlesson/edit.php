...
/**
 * This file is part of the vrlesson vmodule for Moodle
 *
...
$url = new moodle_url('/mod/vrlesson/edit.php');
if ($rid !== 0) {
    $url->param('rid', $rid);
}
if ($cancel !== '') {
...
if (empty($cm->visible) and !has_capability('moodle/course:viewhiddenactivities', $context)) {
    $strdatabases = get_string("modulenameplural", "vrlesson");
...
if (has_capability('mod/vrlesson:managetemplates', $context)) {
    if (!$DB->record_exists('vrlesson_fields', array('dataid'=>$data->id))) {      // Brand new vrlesson!
...
if (!has_capability('mod/vrlesson:manageentries', $context)) {
    if ($rid) {
        ...
        if (!vrlesson_isowner($rid) || vrlesson_in_readonly_period($data)) {
            print_error('noaccess','vrlesson');
        }
    } else if (!vrlesson_user_can_add_entry($data, $currentgroup, $groupmode, $context)) {
...
if (!empty($CFG->enablerssfeeds) && !empty($CFG->vrlesson_enablerssfeeds) && $data->rssarticles > 0) {
    $rsspath = rss_get_url($context->id, $USER->id, 'mod_vrlesson', $data->id);
...
$possiblefields = $DB->get_records('vrlesson_fields', array('dataid'=>$data->id), 'id');
...
if ($rid) {
    $PAGE->navbar->add(get_string('editentry', 'vrlesson'));
}
...
if ($datarecord = data_submitted() and confirm_sesskey()) {
    ...
    if ($rid) {                                          /// Update some records
        ...
        $record = $DB->get_record('vrlesson_records', array('id'=>$rid));
        ...
        $DB->update_record('vrlesson_records', $record);
        ...
        foreach ($datarecord as $name => $value) {
            if (!in_array($name, $ignorenames)) {
                ...
                if (empty($field->field) || ($namearr[1] != $field->field->id)) {  // Try to reuse classes
                    $field = vrlesson_get_field_from_id($namearr[1], $data);
                }
                if ($field) {
        ...
        add_to_log($course->id, 'vrlesson', 'update', "view.php?d=$data->id&amp;rid=$rid", $data->id, $cm->id);

        redirect($CFG->wwwroot.'/mod/vrlesson/view.php?d='.$data->id.'&rid='.$rid);
        ...
        if ($emptyform){    //nothing gets written to database
            echo $OUTPUT->notification(get_string('emptyaddform','vrlesson'));
        }

        if (!$emptyform && $recordid = vrlesson_add_record($data, $currentgroup)) {    //add instance to vrlesson_record
            ...
            /// For each field in the add form, add it to the vrlesson_content.
            foreach ($datarecord as $name => $value){
                if (!in_array($name, $ignorenames)) {
                    $namearr = explode('_', $name);  // Second one is the field id
                    if (empty($field->field) || ($namearr[1] != $field->field->id)) {  // Try to reuse classes
            ...
            add_to_log($course->id, 'vrlesson', 'add', "view.php?d=$data->id&amp;rid=$recordid", $data->id, $cm->id);

            if (!empty($datarecord->saveandview)) {
                redirect($CFG->wwwroot.'/mod/vrlesson/view.php?d='.$data->id.'&rid='.$recordid);
            }
        }
    }
...
echo $OUTPUT->header();
groups_print_activity_menu($cm, $CFG->wwwroot.'/mod/vrlesson/edit.php?d='.$data->id);
...
if (!$rid){
    echo $OUTPUT->heading(get_string('newentry','vrlesson'), 2);
}
...
if ($data->addtemplate){
    ...
    foreach ($possiblefields as $eachfield){
        $field = vrlesson_get_field($eachfield, $data);
    ...
    echo vrlesson_generate_default_template($data, 'addtemplate', $rid, true, false);
    $newtext = '';
}

echo $newtext;

echo '<div class="mdl-align"><input type="submit" name="saveandview" value="'.get_string('saveandview','vrlesson').'" />';
if ($rid) {
    echo '&nbsp;<input type="submit" name="cancel" value="'.get_string('cancel').'" onclick="javascript:history.go(-1)" />';
} else {
    if ((!$data->maxentries) || has_capability('mod/vrlesson:manageentries', $context) || (vrlesson_numentries($data) < ($data->maxentries - 1))) {
        echo '&nbsp;<input type="submit" value="'.get_string('saveandadd','vrlesson').'" />';
    }
}
echo '</div>';
echo $OUTPUT->box_end();
