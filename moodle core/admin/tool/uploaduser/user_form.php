...
class admin_uploaduser_form1 extends moodleform {
    function definition () {
        ...
        $myString = '<span style="font-size:12px;" >Click on the help icon for an example format</span><br />';
        $mform->addElement('html', $myString); 

        $myString = '<div class="examplecsv" ><span>Download an example CSV file: </span>
                    <a href="/mod/resource/view.php?id=16" >Example </a></div>';
        $mform->addElement('html', $myString); 
        ...
        // Let the user choose which course to upload users for
        //---------------------------
        $courseList = array();
        $courseList[0] = "";
        if ($courses = enrol_get_my_courses(NULL, 'visible DESC, fullname ASC'))
        {
            ...
            {
                $courseList[$singleCourse->id] = $singleCourse->shortname;
            }
        }
        $courseBoxName = "Choose course";
        $mform->addElement('select', 'courselist', $courseBoxName, $courseList);
...
class admin_uploaduser_form2 extends moodleform {
    function definition () {
        ...
        // Rachel Fransen - make sure we pass the course id from first selection box
        //--------------------------------------------
        $mform->addElement('hidden', 'courselist');  
        $mform->setType('courselist', PARAM_INT);
